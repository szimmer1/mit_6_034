...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for term_i in expr1:
            for term_j in expr2:
                terms.append( Product([term_i, term_j]).simplify() )
        return Sum(terms).simplify()
    elif isinstance(expr1, Product) and isinstance(expr2, Product):
        # combine everything (append)
        return Product([expr1, expr2]).flatten()
    else:   # one is a product, the other is a Sum
        prod, sum = (expr1,expr2) if isinstance(expr1, Product) else (expr2, expr1)
        terms = []
        for term in sum:
            terms.append( Product([prod, term]).simplify() )
        return Sum(terms).simplify()